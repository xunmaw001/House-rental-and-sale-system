<template>
  <div class="main-content">
		    ...
		    label="身份证">
		     <template slot-scope="scope">
                ...
                <el-button v-if="isAuth('yonghu','查看') && contents.tableBtnIcon == 0" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                                                                <el-button v-if="isAuth('yonghu','租售合同') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="zushouhetongCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'租售合同':'' }}</el-button>
                <el-button v-if="isAuth('yonghu','租售合同') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="zushouhetongCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'租售合同':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('yonghu','租售合同') && contents.tableBtnIcon == 0" type="success" size="mini" @click="zushouhetongCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'租售合同':'' }}</el-button>
    ...
    <add-or-update v-if="addOrUpdateFlag" :parent="this" ref="addOrUpdate"></add-or-update>
            ...
            <zushouhetong-cross-add-or-update v-if="zushouhetongCrossAddOrUpdateFlag" :parent="this" ref="zushouhetongCrossaddOrUpdate"></zushouhetong-cross-add-or-update>
...
import zushouhetongCrossAddOrUpdate from "../zushouhetong/add-or-update";
export default {
  data() {
    return {
                                                                                                                                                                                    searchForm: {
      ...
      addOrUpdateFlag:false,
                  zushouhetongCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    contentPageStyleChange(){
      ...
      this.contents.pageEachNum = 10
            ...
            zushouhetongCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.zushouhetongCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','yonghu');
      this.$nextTick(() => {
        this.$refs.zushouhetongCrossaddOrUpdate.init(row.id,type);
      });
    },
                    init () {
                                                                                                                                                                                                                                                            },
